...
class Student:
    ...
    def __init__(self, first_name: str, middle_name: str, last_name: str, subjects_file='subjects.csv') -> None:
        ...
        self.perfomance_journal = {
            subject: {'marks': [], 'test_marks': [], 'avg_mark': None, 'avg_test_mark': None} for subject in self.subjects}
    ...
    def __str__(self) -> str:
        ...
        line = '-'*70
        subject_stat = '\n'.join([f"{sub:<20}\t{self.perfomance_journal[sub]['avg_mark']:<15}\t{self.perfomance_journal[sub]['avg_test_mark']:<15}"
    ...
    def set_subject_mark(self, subject: str, mark: int, is_test=False):
        ...
        self.validate_mark(
            mark, max_mark=self.MAX_TEST_MARK) if is_test else self.validate_mark(mark, min_mark=self.MIN_MARK)
    ...
    def _update_subject_stat(self, subject: str, is_test: bool = False) -> None:
        ...
        self.perfomance_journal[subject][avg_subject_mark] = sum(
            self.perfomance_journal[subject][subject_marks]) / len(self.perfomance_journal[subject][subject_marks])

    def _update_total_avg(self, is_test):
        '''Обновляет общую среднюю оценку по всем предметам'''
        avg_subject_mark, total_avg_mark = (
            'avg_test_mark', 'total_avg_test_mark') if is_test else ('avg_mark', 'total_avg_mark')
                ...
                for sub_key, sub_val in val.items():
                    if sub_key == avg_subject_mark:
                        if sub_val is not None:
                            total_avg += sub_val
        self.perfomance_journal[total_avg_mark] = total_avg / \
            len(self.subjects)

    def export_journal_to_json(self, file_name: str = 'perfomance_journal', preaty_print: bool = False):
        '''Экспортирует журнал успеваемости в json файл'''
        export_file = f"{file_name}.json"
        write_json(st.perfomance_journal, export_file, preaty_print)
...
def fill_random_journal(st: Student, tests_count: int) -> None:
    ...
    for sub in st.subjects:
        for _ in range(tests_count):
            st.set_subject_mark(sub,
                                random.randrange(2, 5 + 1))
        for _ in range(tests_count // 8):
            st.set_subject_mark(sub,
                                random.randrange(100 + 1), is_test=True)
